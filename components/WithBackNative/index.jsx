...
const WithBackNative = ({
  ...
  const getExtInfo = () => {
    ...
    if (isAndroid) {
      param = {className: 'com.yuge.yixin.ActivityUtils', methodName: 'startCustomWebView', ext01: {key: 'mActivity', type: 'Class', value: 'android.content.Context'}, ext02: {key: 'customTitle', type: 'String', value: ''}, ext03: {key: 'url', type: 'String', value: encodeURI(articleUrl)}}
    }
  ...
  const getSchemeUrl = () => {
    ...
    if (deviceTypeName === 'android') {
      url = `yugeapp://m.yuge.com/qrcode/url/action/redirect.htm?data=${JSON.stringify({url: encodeURIComponent(articleUrl)})}`
    }

    if (deviceTypeName === 'ios') {
      url = `https://www.yuge.com/yugemedical/appointPage?${JSON.stringify({url: encodeURIComponent(articleUrl)})}`
    }
  ...
  // app 端
  if (clientName === 'app') {
    return onClick ? React.cloneElement(children, {
      onClick,
    }) : children
  }
